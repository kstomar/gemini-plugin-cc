...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model>] [what Gemini should investigate, solve, or continue]"
...
`gemini:gemini-rescue` is a subagent, not a skill — do not call `Skill(gemini:gemini-rescue)` or `Skill(gemini:rescue)` (re-enters this command and hangs the session). The command runs inline so the `Agent` tool stays in scope; forked general-purpose subagents do not expose it.
