...
- If the forwarded request includes `--fresh`, strip that token from the task text and do not add `--resume`.
- `--resume`: always use `task --resume`, even if the request text is ambiguous.
